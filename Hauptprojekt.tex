...
0) Verifying a pact between metadata-client and metadata-service - Get a metadata entry returns a response which has a matching body
      $.body.additionalTypeSpecificData -> Type mismatch: Expected Map Map(bottomRightBound -> Map(lat -> 1954648900, lng -> 1613291745), topLeftBound -> Map(lat -> 4134225878, lng -> 3873370324)) but received Null null
...
Bei der Put-Methode mit Absicht falschen Header gestzt:
...
Set a new state for a metadata entry:
    HeaderMismatch - Expected header 'Accept' to have value 'text/plain' but was 'text/html, image/gif, image/jpeg, *; q=.2, */*; q=.2'

The following requests were not received:
Interaction: Set a new state for a metadata entry
	in state None
request:
	method: PUT
	path: /metadata/86fc448c-7cac-48cf-88e1-e3bbdfc2452e/state
...
Im Response Header content-Length auf 1 gesetzt
Verifying a pact between metadata-client and metadata-service
  [Using file /home/alex/Mars/mars-import/metadata-client/target/pacts/metadata-client-metadata-service.json]
  Set a new state for a metadata entry
    returns a response which
...
Consumer nichts getestet, da uninteressant. Post auf Provider Seite funktioniert nicht:
Verifying a pact between metadata-client and metadata-service
  [Using file /home/alex/Mars/mars-import/metadata-client/target/pacts/metadata-client-metadata-service.json]
  createMetadata
    returns a response which
...
Daher keine weiteren Tests

Ojekt vorher gelöscht, daher falscher Statuscode
...
0) Verifying a pact between metadata-client and metadata-service - Delete a metadata entry returns a response which has status code 201
